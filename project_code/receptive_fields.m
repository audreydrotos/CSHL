...
edges2 = timeWindow2(1):binSize:timeWindow2(2);
...
for n = 1:nNeurons
    ...
    for t = 1:nTrials
        ...
        trialSpikes1= neuronSpikes(neuronSpikes >=trialStart + timeWindow(1) & ...
            neuronSpikes < trialStart + timeWindow(2));
        alignedSpikes = trialSpikes - trialStart;
        binnedCounts = histcounts(alignedSpikes, edges);
        

        trialSpikes2= neuronSpikes(neuronSpikes >=trialStart + timeWindow2(1) & ...
            neuronSpikes < trialStart + timeWindow2(2));
        alignedSpikes = trialSpikes - trialStart;
        binnedCounts2 = histcounts(alignedSpikes, edges);

        % interleaved counts
        total_counts(1:2:size(binnedCounts2)*2) = binnedCounts;
        total_counts(2:2:size(binnedCounts2)*2) = binnedCounts2;
...
LGD_neurons = binnedTensor(LGD_idx, :, :);

% preallocate mean array
means = zeros(length(LGD_neurons));
