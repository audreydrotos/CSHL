...
colorSelected = ['r','b','g'];
colorTurned = ['r','b','g'];
% Create a 9x3 color matrix (each row is an RGB color)
colors = zeros(9, 3);

% Red getting lighter (1:4) - from dark red to light red
red_dark = [0.8, 0, 0];      % Dark red
red_light = [1, 0.7, 0.7];   % Light red
for i = 1:4
    t = (i-1) / 3;  % Interpolation factor from 0 to 1
    colors(i, :) = (1-t) * red_dark + t * red_light;
end

% Blue (5)
colors(5, :) = [0, 0, 1];    % Pure blue

% Green getting deeper (6:9) - from light green to dark green
green_light = [0.7, 1, 0.7]; % Light green
green_dark = [0, 0.6, 0];    % Dark green
for i = 6:9
    t = (i-6) / 3;  % Interpolation factor from 0 to 1
    colors(i, :) = (1-t) * green_light + t * green_dark;
end

colorsContrast = colors;
